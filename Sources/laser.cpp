...
bool Laser::detectPortExisting(){
    QList<QSerialPortInfo> ports = QSerialPortInfo::availablePorts();
    for(auto &it : ports){
        if(it.portName() == Settings::laserPort){
            return true;
        }
    }
    return false;
...
QString Laser::detectError(QString data){
    return "";
}

QString Laser::sendData(QString data, int timeout){
    QList<QSerialPortInfo> ports = QSerialPortInfo::availablePorts();
    for(auto &it : ports){
        if(it.portName() == Settings::laserPort){

            QSerialPort port(Settings::laserPort);
            port.setBaudRate(9600);

            if(!port.open(QIODevice::ReadWrite)){
                throw Exception("Không mở được cổng kết nối tới máy khắc laser !\nCổng : " + Settings::laserPort);
            }

            port.write(data.toStdString().c_str());
            if(!port.waitForBytesWritten(timeout)){
                port.close();
                throw Exception("Gửi dữ liệu đến máy khắc laser thất bại !\nCổng : " + Settings::laserPort);
            }

            if(!port.waitForReadyRead(timeout)){
                port.close();
                throw Exception("Máy khắc laser không phản hồi !\nCổng : " + Settings::laserPort);
            }
    ...
    throw Exception("Không có kết nối với máy laser trên cổng " + Settings::laserPort);
}

bool Laser::setProgram(QString name){
...
bool Laser::setupBlockData(QString program, QMap<int, QString> data){
    QStringList items;
    items.push_back("C2");
...
bool Laser::setupPosition(QString program, Position pos, int stopper, Design design){
    if(pos.angle % 90 != 0){
        QStringList items = {
...
bool Laser::burn(){
    ...
    if(fb == "NT,0") return true;
        ...
        throw Exception("Lệnh in thất bại.\nMã lỗi : " + code);
    }
    return true;
}
