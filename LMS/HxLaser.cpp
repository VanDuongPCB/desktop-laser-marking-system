...
namespace
{
    HxLaserMachine s_instance;
}


bool HxLaserMachine::detectPortExisting()
{
    QList<QSerialPortInfo> ports = QSerialPortInfo::availablePorts();
    for ( auto& it : ports )
    {
        if ( it.portName() == NxSettings::laserPort )
        {
            return true;
        }
    }
...
QString HxLaserMachine::detectError( QString data )
{
    return "";
}

QString HxLaserMachine::sendData( QString data, int timeout )
{
    QList<QSerialPortInfo> ports = QSerialPortInfo::availablePorts();
    for ( auto& it : ports )
    {
    ...
    throw HxException( "Không có kết nối với máy laser trên cổng " + NxSettings::laserPort );
}

bool HxLaserMachine::setProgram( QString name )
{
...
bool HxLaserMachine::setupBlockData( QString program, QMap<int, QString> data )
{
    QStringList items;
    items.push_back( "C2" );
...
bool HxLaserMachine::setupPosition( QString program, HxPosition pos, int stopper, HxDesign design )
{
    if ( pos.angle % 90 != 0 )
    {
        QStringList items = {
